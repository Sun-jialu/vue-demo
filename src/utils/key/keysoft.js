...
function key_operatorsoft($scope, $http) {	
	var rs;
	var orgNo; 
	var orgId;
	var level = 1;
	...
	$scope.createRootKey = function(motherKeyNode) {
		...
		var psw1 = "pwd1="+motherKeyNode.PSW1+" ;";
		var psw2 = "pwd2="+motherKeyNode.PSW2+" ;";
		try{
			if(motherKeyNode.CARDTYPE == 1){
				tryType = 251;
				var param = psw1+psw2;
				rs = psam.SendCmd(tryType,param);
			}else{
				rs = psam.GetMotherKey(motherKeyNode.PSW1,motherKeyNode.PSW2);
			}			
			if(asvalue(rs)){
				motherKeyNode.MOTHERKEY=value;
				ajaxPost(null, add_url, motherKeyNode,$scope.addFromcallback2, $scope.valideError);
			}
		}catch(e){
			$scope.showbutton(2);
			$scope.valideError("访问控件异常");
		}
	}
	//生成com/orgkey
	$scope.createOrgKey = function(orgKeyNode){
		...
		try{
			...
			if(level == 1){
				...
				var param = $scope.motherkey+","+"DivGene="+orgNo+" ;";
				rs = psam.SendCmd(tryType,param);
			}else if(level == 2){
				orgKeyNode.level = 2;
				...
				var param = $scope.comkey+","+"DivGene="+orgNo+" ;";
				rs = psam.SendCmd(tryType,param);
			}else if(level == 3){
				orgKeyNode.level = 3;
				if($scope.motherkey == undefined ){
				...
				orgKeyNode.orgid=$scope.orgKeyNode.orgid;
				var orgSectors="";
				$('#jiami input[type="checkbox"]:checked').each(function(){ 
					orgSectors+=this.value +","; 
				});
				if(orgSectors == ""){
					$scope.valideError("请选择加密扇区");
					$scope.showbutton(2);
					return;
				}
				if(orgSectors.substring(0,orgSectors.indexOf(",")) == "on"){
					orgKeyNode.ORGSECTORS=orgSectors.substring(orgSectors.indexOf(",")+1,orgSectors.length-1);
				}else{
					orgKeyNode.ORGSECTORS=orgSectors.substring(0,orgSectors.length-1);
				}	
				var cardSector = "";
				$('#faka input[type="checkbox"]:checked').each(function(){ 
					cardSector+=this.value +","; 
				});
				if(cardSector == ""){
					$scope.valideError("请选择两个发卡扇区进行发卡");
					$scope.showbutton(2);
					return;
				}
				cardSector = cardSector.substring(0,cardSector.length-1);
				var array = cardSector.split(",");
				if(array.length!=2){
					$scope.valideError("发卡扇区只能选择两个");
					$scope.showbutton(2);
					return;
				}				
				for(var k = 0 ;k<array.length ; k++){
					if(orgKeyNode.ORGSECTORS.indexOf(array[k])<0){
						$scope.valideError("发卡扇区必须是加密过的扇区");
						$scope.showbutton(2);
						return;
					}
				}
				$scope.cardsector = cardSector;
				orgKeyNode.CARDSECTORS=cardSector;
				rs = psam.GenCommunityKey($scope.motherkey,orgNo);
			}			
			if(asvalue(rs)){
				orgKeyNode.key = value;
		...
		$.blockUI({
			...
			onBlock : function() {
				if(orgKeyNode.type == 1){
					...
					if(result[0] == 0){
						tranType = 255;
						rs = psam.SendCmd(tranType,$scope.orgkey);			
						result = rs.split(",");
						if(result[0] == 0){
							psam.ReleaseReader();
							if(result[0] != 0){
