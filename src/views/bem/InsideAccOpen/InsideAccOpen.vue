      ...
      <el-row>
        <el-col :span="8">
          <el-form-item label="培训机构:">
            <el-select v-model="addForm.subOrgId" placeholder="请选择">
              <!-- <el-option
                :label="item.dictValue"
                :value="item.dictCode"
                v-for="(item,index) in orgIdNameArr"
                :key="index"
              ></el-option>-->
              <el-option label="新东方培新学校" value="1"></el-option>
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item label="内部账户名称:" prop="subAccName">
            <el-input
              maxlength="20"
              show-word-limit
              placeholder="请输入字符在20字以内"
              v-model="addForm.subAccName"
            ></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="8">
  ...
  data() {
    return {
      ...
      rules: {
        ...
        //内部账户名称
        subAccName: [
          { required: true, message: "内部账户名称不能为空", trigger: "blur" },
          {
            min: 0,
            max: 20,
            message: "长度在0-20 个字符以内",
            trigger: "blur"
          }
        ]
      }
