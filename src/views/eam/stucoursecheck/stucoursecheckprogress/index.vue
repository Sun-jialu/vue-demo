    ...
    <div v-if="addOrUpdateIsShow">
        ...
        <el-row>
           ...
           <el-col :span="8">
             ...
             <el-col :span="8">
                 <el-form-item label="批阅状态：" prop="state">
                    <el-select v-model="formData.state" clearable placeholder="请选择批阅状态">
                        <el-option
                        :label="item.name"
                        :value="item.value"
...
<script>
 ...
 import { getPageListByParam} from "@/api/eam/stucoursecheckprogress";
export default {
    components: {
        addOrUpdate
    },
    ...
    //获取下拉框
     extIdName() {
      ...
      getDropDownList({"code":"eam_schools"}).then(res => {
       ...
       //批阅状态
      getConstantDownList({"code":"workstate"}).then(res => {
        if (res && res.rsCode == "AAAAAAA") {
          let data = res.data;
          this.stateArr = data;
