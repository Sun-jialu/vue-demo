  ...
  <div class="eamCoursechapter">
    ...
    <div v-if="addOrUpdateIsShow">
        ...
        <el-row>
            ...
            <!-- <el-col :span="8">
                <el-form-item label="所属章节：" prop="courseId">
                    <el-select v-model="formData.courseId" clearable placeholder="请选择所属章节">
        ...
        <div id="print">
            ...
            label="操作" fixed="right" align="center" width="100">
                <template slot-scope="scope">
                    <el-button type="text" size="small" @click="addNew(scope.row.chapterId,scope.row.schoolId)">编辑</el-button>
                    <el-button type="text" size="small" @click="deleteColumn(scope.row.chapterId,scope.row.schoolId)">删除</el-button>
                </template>
            </el-table-column>
            </el-table>
        </div>
...
export default {
  ...
  methods: {
    ...
    deleteColumn(id,schoolId){
        this.$confirm('确定删除该条数据?', '提示', {
          ...
          type: 'warning'
             ...
             let arrid=[]
            arrid.push(id)
           deleteData({"chapterIds":arrid,"schoolId":schoolId}).then(res => {
                if (res.rsCode=== 'AAAAAAA') {
                    this.queryAbout()
                }
            })
    ...
    exportExcel() {
      ...
      let url=`${eamUrl}/eam/courseChapter/exportData`
